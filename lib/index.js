...
function _adjustAdjacencyMatrices(props, input) {
  ...
  for (var j = 0, l2 = edgeLabelKeys.length; j < l2; j++) {
    ...
    for (var i = 0, l = edges.length; i < l; i++) {
      ...
      adjacencyMatrix[mappedSourceId].push({target: mappedTargetId, weight: weight, sourceNode: nodesIdMap[source], targetNode: nodesIdMap[target]});
      if (!adjacencyMatrix[mappedTargetId]) {
        adjacencyMatrix[mappedTargetId] = [];
      }
      adjacencyMatrix[mappedTargetId].push({source: mappedSourceId, weight: weight, sourceNode: nodesIdMap[source], targetNode: nodesIdMap[target]});
    }
  }
}
